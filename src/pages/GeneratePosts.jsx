    ...
    try {
      ...
      if (contentType === 'text') {
        ...
        try {
          ...
          // Verify that images were actually generated
          if (!newPosts || newPosts.length === 0 || !newPosts[0].imageUrl) {
          ...
          newPosts = [
            {
              text: "📸 NEW: Our revolutionary business automation dashboard! See how easy it is to manage everything in one place! 🎯",
              platform: "Instagram",
              type: "image",
              ...
              hashtags: "#Dashboard #Automation #BusinessTools #Innovation #Technology #DigitalTransformation #Efficiency #Results #Future #Leadership #Excellence #Motivation #Inspiration #Achievement #Success",
              aiGenerated: false
            },
            {
              text: "🖼️ Infographic: The Ultimate Guide to Business Growth in 2024! Save and share with your team! 📊",
              platform: "Facebook",
              type: "image",
              imageUrl: `https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=512&h=512&fit=crop&crop=center&auto=format&q=80&sig=${Date.now()}`,
              hashtags: "#Infographic #BusinessGrowth #2024 #Innovation #Success #Professional #Technology #DigitalTransformation #Efficiency #Results #Future #Leadership #Excellence #Motivation #Inspiration",
              aiGenerated: false
            },
            {
              text: "📱 Carousel Post: 5 Steps to Automate Your Business Today! Swipe through to see each step! 🔄",
              platform: "Instagram",
              type: "carousel",
              images: [
                "https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=512&h=512&fit=crop&crop=center&auto=format&q=80",
                "https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=512&h=512&fit=crop&crop=center&auto=format&q=80",
                "https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=512&h=512&fit=crop&crop=center&auto=format&q=80"
              ],
              hashtags: "#Carousel #Automation #BusinessSteps #Innovation #Success #Professional #Technology #DigitalTransformation #Efficiency #Results #Future #Leadership #Excellence #Motivation",
              aiGenerated: false
            }
          ];
        }
                ...
                <div className="space-y-6">
                  {posts.map((post, idx) => (
                    <div key={idx} className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition-all duration-300">
                      ...
                      {post.type === 'text' && (
                        <div className="mb-4">
                          <p className="text-white text-lg leading-relaxed mb-2">{post.text}</p>
                          {post.hashtags && (
                            <p className="text-purple-300 text-sm leading-relaxed">{post.hashtags}</p>
                          )}
                        </div>
                      )}
                      ...
                      <div className="flex justify-between items-center">
                        <span className="text-xs text-emerald-300">
                          {`${post.text}${post.hashtags ? '\n\n' + post.hashtags : ''}`.length} characters
                        </span>
                        <button
                          onClick={() => navigator.clipboard.writeText(`${post.text}${post.hashtags ? '\n\n' + post.hashtags : ''}`)}
